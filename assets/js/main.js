...
data.forEach(element => {
    document.getElementById("content").innerHTML += `<img src="${element.url}" alt=""><h3>${element.question}</h3>`
    document.getElementById("content").innerHTML += `<br> <h1 id="a${counterAnswer}" value="${element.answer}">Answer</h1>`
    element.choice.forEach(inner => {
        document.getElementById("content").innerHTML += `<input type="button" value="${inner}" id="a${counterAnswer}b${counterChoice}">`
...
document.querySelectorAll('[type = "button"]').forEach(item => {
    item.addEventListener("click", function () {
        let i = item.id.substring(1, 2)
        let j = item.id.substring(3, 4)
        console.log("click")

        if (data[i].choice[j] == data[i].answer) {
            document.getElementById(`a${i}`).innerHTML = `<b style="color:green">CORRECT</b>`
            item.style.background = "green"
            item.style.color = "#000000"
            console.log("correct!", "\nyour choice: \t\t" + data[i].choice[j], "\ncorrect answer: \t" + data[i].answer)
            answers++
            correctAnswers++
            console.log("correct Answers: " + correctAnswers)
            document.getElementById('aside').innerHTML = correctAnswers + "/" + answers
        }
        else {
            document.getElementById(`a${i}`).innerHTML = `<b style="color:red">FALSE</b>`
            item.style.background = "red"
            item.style.color = "#000000"
            console.log("false!", "\nyour choice: \t\t" + data[i].choice[j], "\ncorrect answer: \t" + data[i].answer)
            answers++
            console.log("correct Answers: " + correctAnswers)
            document.getElementById('aside').innerHTML = correctAnswers + "/" + answers + "<br> correct"
        }
        for (let x = 0; x < data[i].choice.length; x++) {
            document.getElementById(`a${i}b${x}`).disabled = true
        }
    })
})